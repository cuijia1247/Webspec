...
本仓库用于对 **Web UI 截图/设计稿** 进行 **视觉相似度**、**风格相似度（CLIP patch Gram）**、**布局结构相似度**、**组件框级匹配（CS）**、**内容相关组件 OCR 文本双向指标** 等计算，以及从 SCUT 目录按主文件名过滤拷贝等数据整理工作。
...
3. [脚本运行说明](#脚本运行说明)
   ...
   - [4. CLIP 风格相似度（`style_similarity_calculation.py`）](#4-clip-风格相似度style_similarity_calculationpy)
   - [5. SCUT 产出过滤拷贝](#5-scut-产出过滤拷贝-utilsfilterpy)
   - [6. 五分区布局检测](#6-五分区布局检测-utilsfive_dicts_predictpy)
   - [7. UI 组件框检测](#7-ui-组件框检测-utilscomponents_predictpy)
   - [8. Level-2 组件分区检测](#8-level-2-组件分区检测-utilsregion_predictpy)
   - [9. 布局相似度 LS](#9-布局相似度-ls-layout_similarity_calculationpy)
   - [10. 组件相似度 CS](#10-组件相似度-cs-component_similarity_calculationpy)
   - [11. 文本内容相似度](#11-文本内容相似度-content_similarity_calculationpy)
   - [12. 布局类别说明](#12-布局类别说明-utilsui_layout_instructionyaml)
4. [方法简介](#方法简介)
5. [仓库结构](#仓库结构)
...
# CLIP Gram 风格相关性（style_similarity_calculation.py：斯皮尔曼 / 肯德尔）
pip install scipy
...
### 4. CLIP 风格相似度（`style_similarity_calculation.py`）

基于 **CLIP 视觉编码器** 的 **patch 序列特征**，构造每张图的 **Gram 矩阵**（对 patch 维平均，不含 CLS），将矩阵 **上三角（含对角）** 展平为向量后，计算 source 与 target 之间的三项 **风格相关性** 指标（越大表示两幅图在「CLIP 二阶统计」上越一致；与整图 CLIP 余弦语义相似度不同）：

- **style_cos**：余弦相似度
- **style_pearman**：斯皮尔曼相关系数（需 `scipy`）
- **style_kendell**：肯德尔 τ（需 `scipy`）

依赖：在「环境与依赖」中安装 **`pip install scipy`**。

**默认：单图对**（`-s` / `-t` 默认与 `visual_similarity_calculation.py` 一致）：

```bash
python style_similarity_calculation.py
python style_similarity_calculation.py -s path/source.png -t path/target.jpg
```

- 默认报告：**`output/style_similarity/style_analysis.md`**（可用 `-o` 覆盖相对仓库根的路径）。

**`--dir`：文件夹批量**（`--source-dir` / `--target-dir`，stem 配对规则与视觉相似度脚本一致）：
...
- 批量时同一文件内逐行记录每对的 **style_cos / style_pearman / style_kendell**，并在文末给出三项 **平均值**。

| 参数 | 默认 | 说明 |
|------|------|------|
| `-s` / `--source` | 与 `visual_similarity_calculation.py` 单图默认一致 | 单图：source 路径 |
| `-t` / `--target` | 同上 | 单图：target 路径 |
| `-o` / `--output` | `output/style_similarity/style_analysis.md` | 输出 `.md`；相对路径相对仓库根 |
| `--dir` | 关闭 | 启用文件夹批量 |
| `--source-dir` | `data/QwenVL/snapshot` | 批量：source 图目录 |
| `--target-dir` | `data/images_origin` | 批量：target 图目录 |
| `--model`、`--cache_dir`、`--proxy`、`--hf_endpoint`、`--device` | 与 `vs_clip_score` / `visual_similarity_calculation` 一致 | Hugging Face CLIP 与推理设备 |

---

### 5. SCUT 产出过滤拷贝（`utils/filter.py`）
...
### 6. 五分区布局检测（`utils/five_dicts_predict.py`）
...
### 7. UI 组件框检测（`utils/components_predict.py`）
...
### 8. Level-2 组件分区检测（`utils/region_predict.py`）
...
### 9. 布局相似度 LS（`layout_similarity_calculation.py`）
...
### 10. 组件相似度 CS（`component_similarity_calculation.py`）
...
### 11. 文本内容相似度（`content_similarity_calculation.py`）
...
### 12. 布局类别说明（`utils/ui_layout_instruction.yaml`）
...
| CLIP Gram 风格 | `style_similarity_calculation.py` | 同上 | patch Gram → style_cos / style_pearman / style_kendell，默认 `style_analysis.md` |
...
CLIP / DINOv2 输出 **L2 归一化余弦相似度**（0–1，越大越相似）。**风格脚本**输出为 Gram 向量上的余弦与秩相关，数值尺度与整图 CLIP 余弦 **不可直接对比**。布局 **LS** 为树编辑距离之和（越小越相似）；组件 **CS** 为两段匹配率之和（每项约 0–1，**CS** 约 0–2，越大框级组件越对齐）。CLIP 指标与 LS、CS **不宜混比绝对值**。
...
├── style_similarity_calculation.py   # CLIP patch Gram：style_cos/pearman/kendell → output/style_similarity/
...
│   ├── style_similarity/             # 风格相关性 style_analysis.md
